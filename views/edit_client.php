...
$titleOfModifyModal = "Modification du client";
$titleOfAddModal = "Ajout du client";
$titleOfDeleteModal = "Confirmation";
$urlOfModifyModal = "/modifier/client";
$urlOfAddModal = "/ajouter/client";
$urlOfDeleteModal = "/supprimer/client";
$urlOfSearch = "/rechercher/client";
?>
...
<html lang="fr">
    ...
    <body class="overflow-hidden font-medium">
        <div class="grid grid-cols-6 gap-2  bg-gray-200">
            ...
            <div class="col-span-5">
                ...
                <div class="flex justify-between flex-wrap bg-white p-4 m-3 rounded">
                    ...
                    <div class="">
                        <a href="/editer/client" class="bg-white px-4 py-2 border focus:outline-none suppl-active rounded" id="client-suppl-input">Client</a>
                        <a href="/editer/voiture" class="bg-white px-4 py-2 border focus:outline-none suppl-non-active rounded" id="car-suppl-input">Voiture</a>
                ...
                <div class="text-gray-600 overflow-y-auto m-3 p-4 bg-white scrollbar-m rounded h-fix">
                        ...
                        <tbody class="">
                            <?php
                                for ($i = 0; $i < count($clientData); $i++) {
                                    echo "<tr class='hover:bg-gray-100 data-client'>";
                                    echo "<td class='px-3 py-1.5'>{$clientData[$i]['idcli']}</td>";
                                    echo "<td class='px-3 py-1.5'>{$clientData[$i]['nom']}</td>";
                                    echo "<td class='px-3 py-1.5'>{$clientData[$i]['contact']}</td>";
                                    echo "<td class='px-3 py-1.5'><a href='#' class='px-1 py-1.5 mod-item mr-2 rounded' mod-client-data-id='{$clientData[$i]['idcli']}'><i class='bi bi-pencil'></i></a><a href='#' class='px-1 py-1.5 del-item ml-2 rounded' del-client-data-id='{$clientData[$i]['idcli']}'><i class='bi bi-x-lg'></i></a></td>";
                                    echo "</tr>";
                                }
                            ?>
                        </tbody>
        ...
        <?php include_once "views/components/modify_client_modal.php"; ?>

        <!-- fenêtre modal pour l'ajout du client désiré -->
        <?php include_once "views/components/add_client_modal.php"; ?>
        ...
        <script type="text/javascript" src="/views/assets/js/script_edit_client.js"></script>
    </body>
</html>
