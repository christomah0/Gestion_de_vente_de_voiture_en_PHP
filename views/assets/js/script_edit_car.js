...
const mCarForm = document.querySelector("#m-car-form");
const aCarForm = document.querySelector("#a-car-form");
...
const modifyLinksCar = document.querySelectorAll("[mod-car-data-id]");
...
searchInput.addEventListener("keyup", function(e) {
    ...
    if (charObtained != "") {
        ...
        delBtn.addEventListener("click", function() {
            ...
            showDataMatchedInSearchInput(".data-car", charObtainedToLowerCase, 1);
        });
        showDataMatchedInSearchInput(".data-car", charObtainedToLowerCase, 0);
    } else {
        delBtn.classList.replace("inline", "hidden");
        showDataMatchedInSearchInput(".data-car", charObtainedToLowerCase, 1);
    }
...
mCarForm.addEventListener('submit', (e) => {
    // e.preventDefault();
    hideModalAferSubmission(".modal-input", "#m-modal");
});

aCarForm.addEventListener('submit', (e) => {
    // e.preventDefault();
...
modifyLinksCar.forEach((link) => {
    link.addEventListener("click", (e) => {
        e.preventDefault();
        const id = link.getAttribute("mod-car-data-id");
        showAndHideModal("#m-modal");
        document.getElementById("m-car-id").value = id;
    });
});
...
document.querySelectorAll("[del-car-data-id]").forEach(link => {
    link.addEventListener("click", e => {
        e.preventDefault();
        const id = link.getAttribute("del-car-data-id");
