...
const mClientForm = document.querySelector("#m-purchase-form");
const aClientForm = document.querySelector("#a-purchase-form");
...
const modifyLinksClient = document.querySelectorAll("[mod-purchase-data-id]");
const addBtn = document.querySelector("#a-btn");
const generateBillsBtn = document.getElementById("generate-btn");
...
searchInput.addEventListener("keyup", function(e) {
    ...
    if (charObtained != "") {
        ...
        delBtn.addEventListener("click", function() {
            ...
            showDataMatchedInSearchInput(".data-purchase", charObtainedToLowerCase, 1);
        });
        showDataMatchedInSearchInput(".data-purchase", charObtainedToLowerCase, 0);
    } else {
        delBtn.classList.replace("inline", "hidden");
        showDataMatchedInSearchInput(".data-purchase", charObtainedToLowerCase, 1);
    }
...
aClientForm.addEventListener('submit', (e) => {
    // e.preventDefault();
...
modifyLinksClient.forEach((link) => {
    link.addEventListener("click", (e) => {
        e.preventDefault();
        const id = link.getAttribute("mod-purchase-data-id");
        showAndHideModal("#m-modal");
        document.getElementById("m-purchase-id").value = id;
    });
});
...
document.querySelectorAll("[del-purchase-data-id]").forEach(link => {
    link.addEventListener("click", e => {
        e.preventDefault();
        const id = link.getAttribute("del-purchase-data-id");
...
const startDateSuppl = document.getElementById("start-date-suppl");
const endDateSuppl = document.getElementById("end-date-suppl");
const startDateInput = document.getElementById("start-date-input");
const endDateInput = document.getElementById("end-date-input");

startDateSuppl.addEventListener('input', (e) => {
    startDateInput.value = e.target.value;
});

endDateSuppl.addEventListener('input', (e) => {
    endDateInput.value = e.target.value;
});

// envoie des données nécessaire au server pour impression du facture
...
generateBillsBtn.addEventListener('click', () => {
    let data = [];
    Array.from(document.querySelectorAll('.data-purchase')).forEach(row => {
        data.push(row.children.item(0).textContent);
    })
    postDataToServer("/exporter", data);
});
